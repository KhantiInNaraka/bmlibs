...
static inline bool bm_inference (void                      *p_bmrt,
  ...
  std::shared_ptr<bm_tensor_t> input_tensors(new bm_tensor_t[net_info->input_num], [](bm_tensor_t* p) {delete[] p; });
  std::shared_ptr<bm_tensor_t> output_tensors(new bm_tensor_t[net_info->output_num], [](bm_tensor_t* p) {delete[] p; });
  ...
  for (size_t i =0; i < inputs.size(); i++) {
    ...
    if (DATA_TYPE_EXT_FLOAT32 == inputs[i]->data_type) {
      input_tensors.get()[i].dtype = BM_FLOAT32;
    } else if (DATA_TYPE_EXT_1N_BYTE_SIGNED == inputs[i]->data_type) {
      input_tensors.get()[i].dtype = BM_INT8;
    } else if (DATA_TYPE_EXT_1N_BYTE == inputs[i]->data_type) {
      input_tensors.get()[i].dtype = BM_UINT8;
    }  else {
      std::cout << "bm_inference: input type error !" << std::endl;
      return false;
    }

    input_tensors.get()[i].shape = input_shapes[i];

    input_tensors.get()[i].st_mode = BM_STORE_1N;

    /* attach input memory from bm_images to input tensors */
    int batch_number = input_shapes[i].dims[0];
    bm_image_get_contiguous_device_mem (batch_number, inputs[i], &input_tensors.get()[i].device_mem);
  }

  /* do inference, unblock call */
  bm_handle_t bm_handle = (bm_handle_t)bmrt_get_bm_handle (p_bmrt);
  bool ret = bmrt_launch_tensor (p_bmrt, net_name, input_tensors.get(), inputs.size(), output_tensors.get(), outputs.size());
  if (!ret) {
  ...
  if (res != BM_SUCCESS) {
    ...
    for (size_t i =0; i < outputs.size(); i++) {
      bm_free_device (bm_handle, output_tensors.get()[i].device_mem);
    }
    return false;
  }
  ...
  for (size_t i =0; i < outputs.size(); i++) {
    ...
    size_t size_o = bmrt_tensor_bytesize(&output_tensors.get()[i]);
    bm_memcpy_d2s_partial (bm_handle, outputs[i], output_tensors.get()[i].device_mem, size_o);
    bm_free_device (bm_handle, output_tensors.get()[i].device_mem);
  }
