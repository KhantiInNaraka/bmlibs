...
/**
 * \brief bm_add_ref_data add reference data in bmcompiler when compile with compare
 * \param handle of bmcompiler
 * \param reference name
 * \param reference data
 */
void bm_add_ref_data(void *handle, const char *ref_name, const void *ref_data);
...
/**
 * \bridf bm_add_sequence_gen_layer: generate a list of index data from 0 to length - 1
 * \param compiler,
 * \param input_name: input tensor's name
 * \param output_name: output_tensor's name
 * \param output_is_float: if true output data is float32, otherwise output is int32
 */
void bm_add_sequence_gen_layer(
        void* handle,
        const char* input_name,
        const char* output_name,
...
/**
 * @brief set_output_dtype: force input/output data dtype, usually used for fix8b network
 *        if origin_dtype == dtype, nothing changed
 *        for input:  change input(origin_dtype)->GRAPH to new_input(dtype)*scale->input(origin_dtype)->GRAPH
 *        for output: change GRAPH->output(origin_dtype) to GRAPH->output(origin_dtype)*scale->new_output(dtype)
 * @param handle
 * @param io_name: input/output tensor's name
 * @param dtype: the final dtype for inputs/outputs of the whole graph
 */
void  set_io_dtype(
        void* handle,
        const char* io_name,
...
/**
 * @brief bm_add_split_layer: split layer with dynamic split size
 * @param handle
 * @param input_name
 * @param size_name: split size tensor's name
 * @param output_names
 * @param output_num
 * @param axis
 */
void bm_add_split_layer(
        void* handle,
        const char* input_name,
        const char* size_name,
...
/**
 * @brief bm_set_current_layer_tag: set tag to identify original layer type for calibration
 * @param tag
 * @param extra
 */
void bm_set_current_layer_tag(void *handle, const char* tag, const char* extra);
