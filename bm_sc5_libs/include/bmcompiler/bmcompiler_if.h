 ...
 *
 * \param shape_name: crop shape as the tensor with shape_name. if shape_name == NULL, use `output_shape` & `output_shape_dim` instead.
...
/*
 * the difference betw _v2 and the original api is that _v2
 * takes two bottoms as original PriorBox layer:
 * - 1. the final fmap (after a bunch of convolutions)
 * - 2. the original input image
 * NB: what needs from these two bottoms are the width/height of them, not the actual
 * data of the bottoms. The name of the bottoms are kept in order to have dynamic bottoms (with dynamic sizes)
 */
void add_priorbox_cpu_layer_v2(
    void*   p_bmcpl,
    int*    fmap_shape,
    int     fmap_shape_dim,
    char*   fmap_name,
...
/**
 * @brief load a plugin file by name
 * @param filename
 */
void load_plugin(void *p_bmcpl, const char* filename);

/**
 * \brief add_graph_filter
 * \param p_bmcpl: handle from create_bmcompiler
 * \param priority: see FILTER_PRIORITY_XXX in internal/bmcompiler_filters.h
 * \param layer_type: see layer_type in bmcompiler_defs and bmcompiler_net_param.h
 * \param func_ptr: filter function pointer
 * \param name: filter name
 * \param doc: filter introduction
 */

void add_graph_filter(void *p_bmcpl, int priority, int layer_type, void *func_ptr, const char *name, const char *doc);

/**
 * @brief disable_graph_filter
 * @param p_bmcpl
 * \param priority: see FILTER_PRIORITY_XXX in internal/bmcompiler_filters.h
 * \param layer_type: see layer_type in bmcompiler_defs and bmcompiler_net_param.h
 * @param name
 */
void disable_graph_filter(void* p_bmcpl, int priority, int layer_type, const char* name);
